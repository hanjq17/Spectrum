...
def our_flux_forward(
    ...
    ids = torch.concat((txt_ids, img_ids), axis=0)
    ...
    if self.cnt >= self.warmup_steps:
        ...
        if actual_forward:
            ...
            if self.flex_w:
                real_ws = math.floor(min(self.curr_ws, self.num_steps - self.cnt))
                new_w = max(
                    0.0,
                    (math.floor(real_ws) - self.min_ws) / (self.max_ws - self.min_ws) * self.max_w
                )
                feature_info, forecaster = cache_dic['cache']['forecaster']
                forecaster.update_w(new_w)
    ...
    if not actual_forward:
        ...
        hidden_states = torch.concat([encoder_hidden_states, hidden_states], axis=1)
