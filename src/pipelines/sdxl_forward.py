...
def our_sdxl_forward(
    ...
    if not actual_forward:
        ...
        sample = out.reshape(2, sample.shape[1], sample.shape[2], -1).to(sample)  # [2, A, B]
    else:
        current["activated_steps"].append(current["step"])
        
        for index_block, downsample_block in enumerate(self.down_blocks):
            # if index_block == 0:
            #     continue
        ...
        # HERE!!!!!! [2, C, H, W] -> [1, 2*C, H*W]
        feat = sample.reshape(1, sample.shape[0]*sample.shape[1], -1)
